<template>
  <div>
    ...
    <div class="drop">
      ...
      <!-- Trash Medals -->
      <div class="remove"
           v-if="player.medals && player.medals.length"
      >
        <div class="trash"
             @add="trashMedal"
        >
      ...
      <p class="empty"
         v-if="player.medals && !player.medals.length"
      >
        Sem medalhas, arraste e solte aqui para adicionar
      </p>
    </div>
    <!-- Score -->
    <div class="drop">
      ...
      <!-- Trash Scores -->
      <div class="remove"
           v-if="player.scores && player.scores.length"
      >
        <div class="trash"
             @add="trashScore"
        >
      ...
      <p class="empty"
         v-if="player.scores && !player.scores.length"
      >
        Sem pontos, arraste e solte aqui para adicionar
      </p>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      storePlayerScore(scoreId) {
        this.waiting = true;
        const data = { id: scoreId };
        this.$auth.$http.post(`game/${this.gameId}/player/${this.playerId}/score`, data)
          .then(response => {
            this.requestPlayer();
          })
          .catch(error => {
      ...
      destroyPlayerScore(scoreId) {
        this.waiting = true;
        this.$auth.$http.delete(`game/${this.gameId}/player/${this.playerId}/score/${scoreId}`)
          .then(response => {
            this.requestPlayer();
          })
          .catch(error => {
      ...
      attachScore(e) {
        this.storePlayerScore(this.player.scores[e.newIndex].id);
      },
      trashScore(e) {
        this.destroyPlayerScore(this.player.scores[e.newIndex].id);
      }
    },
    mounted() {
